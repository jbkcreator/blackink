...
DDL = [
	...
	# ── Subtask 3.2.3 — Owner Score Self-Serve Landing Page ──────────────────
	# public_booking_url is manually provisioned per INTERNAL_SALES_DEMO rep
	# (a real GHL calendar's public booking page, or a real Google Calendar
	# "Appointment Schedule" public page) — never generated by code; there is
	# no self-serve slot-picker API anywhere in this repo for Google/Microsoft.
	# is_default_sales_booking flags the single connection the landing page
	# and no-show recovery email redirect to — "first row by connection_id"
	# would be an arbitrary, silent choice, so this must be explicit.
	"ALTER TABLE calendar_connections ADD COLUMN IF NOT EXISTS public_booking_url VARCHAR(500)",
	"ALTER TABLE calendar_connections ADD COLUMN IF NOT EXISTS is_default_sales_booking BOOLEAN NOT NULL DEFAULT FALSE",
	"""
	CREATE UNIQUE INDEX IF NOT EXISTS uq_calendar_connections_one_default_sales_booking
		ON calendar_connections ((connection_scope))
