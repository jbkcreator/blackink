...
> ## ⚠ Partly superseded — 2026-09-03
>
> `Week1_Tasks_Dev_Split_v2.md` and `Implementation_Blueprint_v2.md` are the
> Source of Truth **officially applied** to the sprint plan. Where this
> document and those disagree about **scope**, they win.
>
> **What still stands here:** the repo-level findings. Which code paths are
> now dead, which requirements are unmet in the shipped code, and which
> assumptions in the old Dev 3 reference no longer hold. v2 says what to
> build; this says what is currently wrong.
>
> **Confirmed by v2, previously open here:**
> - The Owner Visibility Score is **Dev 2's** (Task 2.1) — §1.1 left the owner
>   unnamed and flagged it as the likely September blocker
> - Touch 1's report is a **PDF attachment**, not a hosted page
> - Touch 5's scarcity angle **survives** as "County Visibility Rank &
>   Scarcity" — §1.4 assumed it died with the metro layer
> - Touch 2's 60 seconds runs from the **Approve click**, not ambiguously from
>   "Touch 1 dispatch"
> - The skip event is named **`touch_skipped_compliance`**
> - Expired-card rejection **is** required (shared DoD item 5) — §3.4's
>   reading that the ground truth omitted expiry was too narrow
> - Approval cards go to **`#blackink-setter`**
...
> ⚠ **The table below is the 2026-09-03 initial assessment and is now stale.**
> 17 of 22 tickets have since closed and tickets 19–22 were added. The map's
> `MAP.md` "Decisions so far" is authoritative for current state; this table is
> kept as the record of what was decided at reconciliation time.
>
> Two items in it are also superseded by corrections above: ticket 11 should
> **remove** the readiness gate rather than rebuild it on the score (§1.1), and
> a new engine-config/holdout ticket is needed (§3.1b).
