...
async def stripe_webhook(request: Request):
	...
	with get_db_context(client_id=client_id) as session:
		...
		if row.payment_auth_hold_payment_intent_id:
			try:
				from src.services.payment_auth import cancel_auth_hold

				cancel_auth_hold(row.payment_auth_hold_payment_intent_id)
			except Exception:
				logger.error(
					"stripe_webhook: failed to cancel $1 auth hold %s for company %s",
					row.payment_auth_hold_payment_intent_id, company_id, exc_info=True,
				)
		session.execute(
			text("UPDATE stripe_webhook_events SET processed_at = NOW() WHERE stripe_event_id = :id"),
			{"id": event["id"]},
